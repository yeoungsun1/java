...
//5번 : 회원가입, 로그인시도, 아이디 중복검사, 로그인 검증 메소드(기능 분리)
public class LoginMain02 {
	...
	public static void main(String[] args) {
		...
		// 1. 회원가입 메소드 호출(매개변수x, 리턴값o) => 변수에 저장, 출력
		ArrayList<User> users = l.signup();
		System.out.println(users);

		// 2. 로그인 시도 메소드 호출(매개변수o, 리턴값x) => 메소드명(인수);
		l.loginAttempt(users);
	}

	// 회원가입 메소드(매개변수x, 리턴타입o-ArrayList)
		...
		for (int i = 1; i <= count; i++) {
			...
			while (true) {
				...
				if (idDuplicate(users, id)) {
					System.out.println("이미 존재하는 아이디입니다. 다시 입려해주세요.");
				} else {
	...
	// 아이디 중복검사 메소드(매개변수 o-ArrayList, String 리턴값 o-boolean)
	...
	// 로그인 시도 메소드(매개변수 o 리턴값 x)
	void loginAttempt(ArrayList<User> users) {
		// 플래그 변수
		boolean loginSuccess = false;

		for (int i = 1; i <= 3; i++) {
			System.out.println("[" + i + "번째 로그인시도]");
		...
		if (!loginSuccess) {
			System.out.println("3번 로그인 실패!! 프로그램이 종료됩니다");
		}
	}

	// 로그인 검증 메소드(매개변수 3개 리턴값 o-int)
